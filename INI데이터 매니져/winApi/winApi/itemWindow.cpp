...
void itemWindow::update(void)
{
	...
	for (int i = 0; i < 3; i++)
	{
		_itemPointX[i] = _rc.left + 26 + i * 40;
...
void itemWindow::render(void)
{
	...
	if (_showInventory && _openCloseinventoryCount == 1)
	{
		...
		sprintf(str, "%d", _vSword.size());
		for (_viSword = _vSword.begin(); _viSword != _vSword.end(); ++_viSword)
		{
			(*_viSword)->render();
		}
		if (_vSword.size() > 0)
		{
			TextOut(getMemDC(), _itemPointX[2], _itemPointY[2], str, strlen(str));
		}
	}
...
void itemWindow::pushItem(int num)
{
	if (hp == num)
	{
	...
	{
		_itemManager->getSword()->setSword(&_itemPointX[2], &_itemPointY[2]);
		_vSword.push_back(_itemManager->getSword());
	}
}
