...
HRESULT bossProgressBar::init(int x, int y, int width, int height)
{
	...
	_y = y;
	_rc = RectMakeCenter(_x, _y, width, height);
	_progressBarDown = IMAGEMANAGER->addImage("bossBarDown","bossHpBarDown.bmp", 0, 0, width, height, true, RGB(255, 0, 255));
	_progressBarUp = IMAGEMANAGER->addImage("bossBarUp", "bossHpBarUp.bmp", 0, 0, width, height, true, RGB(255, 0, 255));
	_width = _progressBarUp->getWidth();
...
void bossProgressBar::release(void)
{
}

void bossProgressBar::update(void)
{
	_rc = RectMakeCenter(_x, _y, _progressBarDown->getWidth(), _progressBarUp->getHeight());
}

void bossProgressBar::render(void)
{
	_progressBarDown->render(getMemDC(), _rc.left + _progressBarDown->getWidth() / 2,
		_y + _progressBarDown->getHeight() / 2, 0, 0, _progressBarDown->getWidth(),
		_progressBarDown->getHeight());
...
void bossProgressBar::setGuage(float currentScore, float maxScore)
{
	_width = (currentScore / maxScore) * _progressBarDown->getWidth();
}
