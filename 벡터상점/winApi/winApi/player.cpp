...
void player::update(void)
{
	...
	if (KEYMANAGER->isOnceKeyDown('3') && !_itemWindow->gethpVec().empty())
	{
		_itemWindow->usingItem(0);
		_currentHp++;
		if (_currentHp > _maxHp) _currentHp = _maxHp;
	}
	if (KEYMANAGER->isOnceKeyDown('4') && !_itemWindow->getSVec().empty())
	{
		_itemWindow->usingItem(1);
		_speed++;
		if (_speed > 5) _currentHp = 5;
	}
...
void player::save(bool save)
{
	...
	{
		char str[128];
		sprintf(str, "%f, %f, %f, %f, %d, %f,",_x,_y,_currentHp,_maxHp,_money,_speed);
		_pSaveLoad.clear();
